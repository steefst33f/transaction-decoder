use std::fs;
use serde_json::Value;

#[test]
fn test_legacy() {
    let raw_transaction_hex = "";
    let actual = transaction_decoder::run(raw_transaction_hex.to_string()).unwrap();
    let expected: String = fs::read_to_string("tests/test_transaction_legacy.json").expect("Couldn't read .json file");

    let expected_json: Value = serde_json::from_str(&expected).expect("Invalid JSON in expected value");
    let actual_json: Value = serde_json::from_str(&actual).expect("Invalid JSON in actual value");
    assert_eq!(expected_json, actual_json);
}

#[test]
fn test_segwit() {
    let raw_transaction_hex = "";
    let actual = transaction_decoder::run(raw_transaction_hex.to_string()).unwrap();
    let expected: String = fs::read_to_string("tests/test_transaction_segwit.json").expect("Couldn't read .json file");

    let expected_json: Value = serde_json::from_str(&expected).expect("Invalid JSON in expected value");
    let actual_json: Value = serde_json::from_str(&actual).expect("Invalid JSON in actual value");
    assert_eq!(expected_json, actual_json);
}